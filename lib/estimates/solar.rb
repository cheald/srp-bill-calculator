...
module SolarEstimate
  ...
  def self.estimate(date, system_size, efficiency, lat, long)
    offset = 0
    system_size *= efficiency

    rise = SunTimes.new.rise(date, lat, long).localtime
    set = SunTimes.new.set(date, lat, long).localtime

    set += 86400 if set < rise
    set -= 86400 if set - rise > 86400

    if date >= rise && date <= set
      ...
      beta = 4
      offset = 1.05
      day_p = (date.to_i - rise.to_i) / (set.to_i - rise.to_i).to_f
      dv = beta_pdf(day_p, beta * offset, beta)
      dm = beta_pdf(0.5 * offset, beta * offset, beta)
      pct = dv / dm * 0.85 * (REL_MONTH_EFF[date.month] / OBS_MONTH_EFF[date.month].to_f).to_f
